...
export async function GET() {
  try {
    ...
    let ownedQuestions = await Question.find({ author: userName })
      .lean()
      .sort({ createdAt: -1 });

    // Fetch questions not authored by the user
    let otherQuestions = await Question.find({ author: { $ne: userName } })
      .lean()
      .sort({ createdAt: -1 });
    ...
    ownedQuestions = ownedQuestions.map(serializeQuestion);
    otherQuestions = otherQuestions.map(serializeQuestion);
